...
plotcfgs <- function(name, cfgs) {
    pdf(file=paste("build/", name, ".pdf", sep=""), width=14)
    plot(c(),
         # xlim is extended a little to leave space for the legend
         xlim=c(0,60+length(row.names(data))), ylim=c(0,max(data, na.rm=TRUE)),
         xlab="n", ylab="runtime(s)", main=paste("n-queens using", name))
    for (i in 1:length(cfgs)) {
        points(data[cfgs[i]], pch=i, col=i, type="b")
    }
    legend("topright", cfgs, pch=1:length(cfgs), col=1:length(cfgs))
}

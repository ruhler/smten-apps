...
void sudoku(const char* board)
{
    ...
    type_t ty = yices_bv_type(4);
    term_t digits[10];
    term_t terms[9][9];

    for (int i = 0; i < 10; i++) {
        digits[i] = yices_bvconst_uint64(4, i);
    }

    // Create a boolean variable for each blank cell.
    for (int r = 0; r < 9; r++) {
        for (int c = 0; c < 9; c++) {
            ...
            if (d >= 0 && d < 9) {
                ...
                yices_assert_formula(ctx, yices_bvlt_atom(terms[r][c], digits[9]));
            }
        }
    }
        ...
        for (int r = 0; r < 9; r++) {
            for (int c = 0; c < 9; c++) {
                ...
                if (d >= 0 && d < 9) {
                    ...
                    int32_t bits[4];
                    yices_get_bv_value(model, terms[r][c], bits);
                    int v = 0;
                    for (int i = 0; i < 4; i++) {
                        v = 2*v + bits[3-i];
                    }

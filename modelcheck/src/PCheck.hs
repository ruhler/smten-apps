...
module PCheck (Model(..), Alg(..), pcheck, pcheck1, pcheck2) where
...
pcheck1 :: (Eq s) => Model s -> (Symbolic s) -> (s -> Bool) -> SMT (Maybe [s])
pcheck1 = pcheck1' 0

pcheck2 :: (Eq s) => Model s -> (Symbolic s) -> (s -> Bool) -> SMT (Maybe [s])
pcheck2 = pcheck2' 0
...
pcheck2' k m mkS p = do
   ...
   ra <- query $ do 
      xs <- replicateM (k+1) mkS
      assert (_I m (head xs) && all (not . _I m) (drop 1 xs) && loopfree m xs)
   case ra of
      Nothing -> return Nothing
      _ -> do
        -- Search backwards for a loopfree path from a failing state
        rb <- query $ do
            xs <- replicateM (k+1) mkS
            assert (loopfree m xs && all p (take k xs) && not (p (last xs)))
        case rb of
          Nothing -> return Nothing
          _ -> do
            -- Search for a failing path of this length

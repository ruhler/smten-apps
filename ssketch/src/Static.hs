  ...
  staticM x@(IntV v) = do
    ...
    case ty of
      ...
      ArrT BitT (ValE (IntV w)) ->
         case v of
           0 -> staticM (arrayV $ BitV False : genericReplicate (w-1) (pad BitT))
           1 -> staticM (arrayV $ BitV True : genericReplicate (w-1) (pad BitT))
           _ -> error $ "cannot use integer literal for bits: " ++ show v
      ArrT t (ValE (IntV w)) -> staticM (arrayV $ x : genericReplicate (w-1) (pad t))
...
typeof (ArrayE a) = do
   ...
   return $ ArrT ty (ValE (IntV (genericLength a)))
